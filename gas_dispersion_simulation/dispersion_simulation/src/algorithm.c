...
void algorithm_init(char const *wind_files_in, int num_snapshots,
		int num_filaments, int area_size_x, int area_size_y, int area_size_z,
		double area_cell_size, double source_x, double source_y,
		double source_z, double spreading_width_std,
		char const *environment_file) {

	algorithm.max_area_size_z = area_size_z;

	// Read the wind field
	flow_init(&algorithm.flow, area_size_x, area_size_y, area_size_z,
			area_cell_size);
	...
	environment_init(&algorithm.environment, area_size_x, area_size_y,
			area_size_z, area_cell_size, environment_file);

	// Initialize hit rate and filament

	hit_rate_init(&algorithm.hit_rate, area_size_x, area_size_y, area_size_z,
			area_cell_size);

	algorithm.filament = malloc(
			num_snapshots * num_filaments * sizeof(struct sFilament));
	...
	algorithm.marker.width = malloc(
			num_snapshots * num_filaments * sizeof(double));
	algorithm.marker.colour = malloc(
			num_snapshots * num_filaments * sizeof(double));
...
void algorithm_simulate_filaments(int num_filaments, double x1, double y1,
		double z1, double x2, double y2, double z2, double spreading_width_std) { //3d
	// Simulate filaments
	int i;
	...
	for (i = 0; i < num_filaments; i++) {
		...
		filament_init(&algorithm.filament[(n - 1) * num_filaments + i], x, y, z,
				algorithm.max_area_size_z, spreading_width_std);
	}
}

// Calculates one hit_rate
int algorithm_hit_rate_calculate(int num_filaments, int num_snapshots,
		double x1, double y1, double z1, double x2, double y2, double z2,
		char *filePath, double spreading_width_std, long long int num_molecules,
		double temperature, double pressure, int unit_choice,
		double windflow_resolution) {	//3d

	algorithm_simulate_filaments(num_filaments, x1, y1, z1, x2, y2, z2,
			spreading_width_std);
	...
	for (z = 0; z <= algorithm.hit_rate.cells_count.z - 1; z++) {
		for (x = 0; x <= algorithm.hit_rate.cells_count.x - 1; x++) {
			for (y = 0; y <= algorithm.hit_rate.cells_count.y - 1; y++) {
				...
				px = x * algorithm.hit_rate.cells_size.x
						+ algorithm.hit_rate.cells_size.x / 2;
				py = y * algorithm.hit_rate.cells_size.y
						+ algorithm.hit_rate.cells_size.y / 2;
				pz = z * algorithm.hit_rate.cells_size.z
						+ algorithm.hit_rate.cells_size.z / 2;

				for (jj = 0; jj < num_filaments * n; jj++) {
					contributedConcentration =
							(num_molecules / pow(algorithm.filament[jj].state.width, 3)) * exp(-sqrt(pow(px - algorithm.filament[jj].state.x, 2)
									                                                               + pow( py - algorithm.filament[jj].state.y, 2)
																								   + pow( pz - algorithm.filament[jj].state.z, 2))
									                                                               / pow( algorithm.filament[jj].state.width, 2));

					hit_rate_add(&algorithm.hit_rate, px, py, pz, contributedConcentration);

					/*hit_rate_add(&algorithm.hit_rate,
					 x * algorithm.hit_rate.cells_size.x,
					 y * algorithm.hit_rate.cells_size.y,
					 z * algorithm.hit_rate.cells_size.z,
					 contributedConcentration);*/
				}

				ppm = concentration(px, py, pz, algorithm.hit_rate.cells_size.x,
						temperature, pressure, unit_choice);
				...
				fprintf(out, "%f   %f   %f   %f   %f   %f   %f\n", px, py, pz,
						ppm, algorithm.flow.u[flow_x][flow_y][flow_z],
						algorithm.flow.v[flow_x][flow_y][flow_z],
						algorithm.flow.w[flow_x][flow_y][flow_z]);
	...
	if (n < num_snapshots) {
		...
		for (nn = 0; nn < steps; nn++)
			for (i = 0; i < num_filaments * n; i++) {
				...
				algorithm.marker.colour[i] = (algorithm.filament[i].state.width
						- 0.04) * 2.0;
...
double concentration(float x_in, float y_in, float z_in, float cell_size,
		double temperature, double pressure, int if_ppm) {
