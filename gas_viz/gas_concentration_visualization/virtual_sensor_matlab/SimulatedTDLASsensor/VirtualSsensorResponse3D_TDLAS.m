function [ totalConcentration ] = VirtualSsensorResponse3D_TDLAS(laserLength, azimuthal_start, azimuthal_end, numAzimuthal, originalX, originalY, originalZ, logFileMatrix, environment )
...
for azimuthal_xy = azimuthal_start:azimuthalStepSize:azimuthal_end
    ...
    while internalIndex<=numBox && hit==0
        ...
        segmentInCell = IntegralMeasurementDecomposition3D(x_startingPoint, y_startingPoint, z_startingPoint, laserCoordinatorX, laserCoordinatorY, laserCoordinatorZ, cellSize, azimuthal_xy, azimuthal_z);
    ...
    concentrationAlongLaser = zeros(size(concentratonInCell));
    concentrationAlongLaser(1:length(segmentInCell)) = segmentInCell;
    
    totalConcentration = concentratonInCell*concentrationAlongLaser';
